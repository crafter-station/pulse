...
export function Heatmap() {
  ...
  const groupByWeeks = (): HeatmapDay[][] => {
    ...
    const paddedData = [...Array(startDayOfWeek)]
      .map(() => ({ date: "", count: -1 }))
      .concat(data);
    ...
    return (
      <div className="bg-[#171717] border-2 border-[#333] p-4 md:p-6">
        <div className="h-32 flex items-center justify-center">
          <div className="text-[#737373] text-sm animate-pulse font-mono uppercase tracking-widest">
            Loading heatmap...
          </div>
        </div>
      </div>
  ...
  return (
    <div className="bg-[#171717] border-2 border-[#333] p-4 md:p-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-3">
        <div>
          <h3 className="text-sm md:text-base font-black text-white mb-1 uppercase tracking-wider">
            [Activity Heatmap]
          </h3>
          <p className="text-[10px] md:text-xs text-[#737373] font-mono uppercase tracking-widest">
            2026 // {totalCommits} total commits
          </p>
        </div>
        <div className="flex items-center gap-1.5 md:gap-2">
          <span className="text-[10px] md:text-xs text-[#737373] font-mono uppercase">
            Less
          </span>
        ...
        <div className="flex gap-0.5 md:gap-1 w-full">
          <div className="hidden sm:flex flex-col gap-0.5 md:gap-1 mr-1.5 md:mr-2 justify-end flex-shrink-0">
            ...
            {[0, 1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="h-2.5 md:h-3 flex items-center justify-end"
              >
                {weekDayIndices.includes(i) && (
                  <span className="text-[8px] md:text-[10px] text-[#737373] pr-1 md:pr-1.5 font-mono uppercase">
                    {weekDays[weekDayIndices.indexOf(i)]}
                  </span>
                )}
              </div>
            ))}
                    ...
                    return (
                      ...
                      >
                        <div className="hidden md:block absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-[#0A0A0A] border-2 border-[#FFD800] text-xs text-white whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 shadow-lg">
                          <div className="font-black text-[#FFD800] mb-0.5">
                            {day.count} commits
                          </div>

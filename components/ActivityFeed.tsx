...
export function ActivityFeed() {
  ...
  const repos = [
    "all",
    ...Array.from(new Set(activity.map((item) => item.repo))),
  ];
  const authors = [
    "all",
    ...Array.from(new Set(activity.map((item) => item.author))),
  ];
  ...
  if (loading) {
    return (
      <section
        id="activity"
        className="py-12 md:py-20 px-4 md:px-6 bg-[#0A0A0A] min-h-[1300px]"
      >
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            <div>
              <h2 className="font-display text-3xl font-black text-white mb-2 uppercase">
                [Recent Activity]
              </h2>
              <p className="text-[#737373] uppercase tracking-widest font-mono text-sm">
                Live feed of all commits to main
              </p>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-[#171717] border-2 border-[#333]">
              ...
              <span className="text-sm text-[#737373] font-mono uppercase">
                Loading...
              </span>
            </div>
          </div>
          <div className="space-y-2">
            {[...Array(8)].map((_, i) => (
              <div
                key={i}
                className="h-16 bg-[#171717]/50 border-2 border-[#262626] animate-pulse"
              />
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
            ...
            <p className="text-sm md:text-base text-[#737373] font-mono uppercase tracking-widest">
              ...
              {filteredActivity.length === 1 ? "commit" : "commits"}
            </p>
          </div>
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="flex items-center gap-2 px-3 md:px-4 py-1.5 md:py-2 bg-[#171717] border-2 border-[#333]">
              <span
                className={`w-2 h-2 ${error ? "bg-red-500" : "bg-green-500 animate-pulse"}`}
              />
              <span className="text-xs md:text-sm text-[#737373] font-mono uppercase tracking-wider">
                {error ? "Error" : "Live"}
              </span>
            </div>
          </div>
        ...
        <div className="max-h-[600px] overflow-y-auto scrollbar-thin">
                ...
                const content = (
                  <>
                    ...
                    <div className="flex-1 min-w-0 flex flex-col md:flex-row md:items-center gap-1 md:gap-3">
                      ...
                      <span
                        className={`text-[#A3A3A3] text-xs md:text-sm font-mono truncate ${item.isPrivate ? "blur-sm select-none" : ""}`}
                      >
                        {item.message}
                      </span>
                    </div>
                    <div className="hidden sm:flex items-center gap-1.5 md:gap-2 text-xs flex-shrink-0">
                      <span className="text-green-500 font-mono text-[10px] md:text-xs font-bold">
                        +{formatNumber(item.additions)}
                      </span>
                      <span className="text-red-500 font-mono text-[10px] md:text-xs font-bold">
                        -{formatNumber(item.deletions)}
                      </span>
                      <span className="text-[#737373] ml-1 md:ml-2 text-[10px] md:text-xs font-mono uppercase">
                        {item.time}
                      </span>
                    </div>
                  </>
                  ...
                  <>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        <span className="font-black text-white text-sm md:text-base uppercase">
                          {item.author}
                        </span>
                        ...
                        {item.isPrivate && <PrivateBadge />}
                        <span className="text-[#737373] text-xs md:text-sm ml-auto font-mono uppercase">
                          {item.time}
                        </span>
                      </div>
                      <p
                        className={`text-[#A3A3A3] font-mono text-xs md:text-sm mb-3 line-clamp-2 ${item.isPrivate ? "blur-sm select-none" : ""}`}
                      >
                        {item.message}
                      </p>
          ...
          {loadingMore && (
            <div className="flex items-center justify-center py-4 gap-2">
              ...
              <span className="text-xs text-[#737373] font-mono uppercase tracking-widest">
                Loading more...
              </span>
            </div>
          )}
